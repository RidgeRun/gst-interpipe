...
/*
 * Given the new property block switch set to TRUE, the interpipesrc wont 
 * switch to another interpipesink.
 */

 /*
  * Given the new property allowing switch set to FALSE, the interpipesrc can 
  * try to connect to another interpipesink.
  */

GST_START_TEST (interpipe_block_switch_on)
{
  GstPipeline *sink1;
  GstPipeline *sink2;
  GstPipeline *src;
  ...
  src =
      GST_PIPELINE (gst_parse_launch
      ("interpipesrc name=intersrc listen-to=sink1 block-switch=true ! "
...
GST_START_TEST (interpipe_block_switch_off)
{
  GstPipeline *sink1;
  GstPipeline *sink2;
  GstPipeline *src;
  ...
  src =
      GST_PIPELINE (gst_parse_launch
      ("interpipesrc name=intersrc listen-to=sink1 block-switch=false ! "
          "appsink name=asink async=false", &error));
  fail_if (error);
  ...
  src2 =
      GST_PIPELINE (gst_parse_launch
      ("interpipesrc name=intersrc listen-to=sink2 block-switch=false ! "
          "appsink name=asink async=false", &error));
  fail_if (error);
...
gst_interpipe_suite (void)
{
  ...
  TCase *tc1 = tcase_create ("block_switch_on");
  TCase *tc2 = tcase_create ("block_switch_off");

  suite_add_tcase (suite, tc1);
  tcase_add_test (tc1, interpipe_block_switch_on);

  suite_add_tcase (suite, tc2);
  tcase_add_test (tc2, interpipe_block_switch_off);
