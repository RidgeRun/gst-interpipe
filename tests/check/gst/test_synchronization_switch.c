 ...
 */
GST_START_TEST (interpipe_synchronization_switch_enable)
{
  GstPipeline *sink1;
  GstPipeline *sink2;
  ...
  src =
      GST_PIPELINE (gst_parse_launch
      ("interpipesrc name=intersrc listen-to=intersink1 synchronization-switch=true block-switch=false allow-renegotiation=true format=3 ! capsfilter caps=video/x-raw,width=[320,1920],height=[240,1080],framerate=(fraction)5/1 ! "
...
gst_interpipe_suite (void)
{
  ...
  TCase *tc1 = tcase_create ("interpipe_synchronization_switch_enable");

  suite_add_tcase (suite, tc1);
  tcase_add_test (tc1, interpipe_synchronization_switch_enable);
